# Ripgrep (`rg`) - Fast File Content Search Tool
...
- **Smart Defaults:** By default, it respects `.gitignore`, `.ignore`, and `.rgignore` files, automatically skipping hidden files, directories, and binary files
...
**To find files containing "your_string_here" recursively in the current directory:**
...
3. **Smart Default Filtering:**
   - **Respects `.gitignore`:** By default, it automatically skips files and directories listed in `.gitignore` files, as well as `.rgignore` and `.ignore` files
